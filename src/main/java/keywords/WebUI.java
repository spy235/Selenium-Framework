/**
 * WebUI is a utility class that provides various functions to interact with web elements
 * using Selenium WebDriver. It includes methods for waiting for elements, clicking elements,
 * taking screenshots, verifying conditions, and more.
 */
package keywords;
...
//            AllureManager.takeScreenshotStep();
    ...
    /**
     * Waits for the page title to match the expected title within a given timeout.
     * 
     * @param expectedTitle   The expected title of the page.
     * @param timeoutInSeconds The maximum time to wait for the title to match, in seconds.
     * @throws Exception If the expected title is not found within the timeout period.
     */
    public static void waitForTitleToBe(String expectedTitle, int timeoutInSeconds) throws Exception {
        int pollInterval = 500; // check every 500ms
        int elapsedTime = 0;
        boolean isTitleMatched = false;

        // Loop until the title matches or the timeout is reached
        while (elapsedTime < timeoutInSeconds * 1000) {
            String actualTitle = WebUI.getPageTitle();

            if (actualTitle.equals(expectedTitle)) {
                isTitleMatched = true;
                break; // Exit the loop when the title matches
            }

            Thread.sleep(pollInterval); // Wait for the poll interval before checking again
            elapsedTime += pollInterval;
        }

        // After the loop, assert that the title matched within the timeout
    ...
    public static void switchToNestedFrame(By parentFrameBy, By childFrameBy) {
        WebDriverWait wait = new WebDriverWait(DriverManager.getDriver(), Duration.ofSeconds(FrameworkConstants.WAIT_EXPLICIT));

        // Switch to the parent frame
        wait.until(ExpectedConditions.frameToBeAvailableAndSwitchToIt(parentFrameBy));

        // Then switch to the child frame
        wait.until(ExpectedConditions.frameToBeAvailableAndSwitchToIt(childFrameBy));
        LogUtils.info("Switched to Nested Frame (Parent: " + parentFrameBy + ", Child: " + childFrameBy + ")");
    }
